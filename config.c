...
int loadConfiguration(const char *filename, ucncAssembly **rootAssembly,
                      ucncLight ***lights, int *lightCount) {
  ...
  mxml_node_t *assembliesNode =
      mxmlFindElement(tree, tree, "assemblies", NULL, NULL, MXML_DESCEND_ALL);
  if (assembliesNode) {
    for (mxml_node_t *assemblyNode =
             mxmlFindElement(assembliesNode, assembliesNode, "assembly", NULL,
                             NULL, MXML_DESCEND_ALL);
         assemblyNode; assemblyNode = mxmlFindElement(
                           assemblyNode, assembliesNode, "assembly", NULL, NULL,
                           MXML_DESCEND_ALL)) {
      ...
      mxml_node_t *originNode = mxmlFindElement(
          assemblyNode, assemblyNode, "origin", NULL, NULL, MXML_DESCEND_ALL);
      if (originNode) {
      ...
      mxml_node_t *positionNode = mxmlFindElement(
          assemblyNode, assemblyNode, "position", NULL, NULL, MXML_DESCEND_ALL);
      if (positionNode) {
      ...
      mxml_node_t *rotationNode = mxmlFindElement(
          assemblyNode, assemblyNode, "rotation", NULL, NULL, MXML_DESCEND_ALL);
      if (rotationNode) {
      ...
      mxml_node_t *colorNode = mxmlFindElement(
          assemblyNode, assemblyNode, "color", NULL, NULL, MXML_DESCEND_ALL);
      if (colorNode) {
      ...
      char motionAxis = ' ';
      int invertMotion = 0;
      mxml_node_t *motionNode = mxmlFindElement(
          assemblyNode, assemblyNode, "motion", NULL, NULL, MXML_DESCEND_ALL);
      if (motionNode) {
      ...
      float minPos = 0.0f, maxPos = 0.0f, minRot = 0.0f, maxRot = 0.0f;
      mxml_node_t *limitsNode = mxmlFindElement(
          assemblyNode, assemblyNode, "limits", NULL, NULL, MXML_DESCEND_ALL);
      if (limitsNode) {
        mxml_node_t *linNode = mxmlFindElement(limitsNode, limitsNode, "linear",
                                               NULL, NULL, MXML_DESCEND_ALL);
        if (linNode) {
          const char *minStr = mxmlElementGetAttr(linNode, "min");
          const char *maxStr = mxmlElementGetAttr(linNode, "max");
          if (minStr)
            minPos = atof(minStr);
          if (maxStr)
            maxPos = atof(maxStr);
        }
        mxml_node_t *rotNode = mxmlFindElement(limitsNode, limitsNode, "rotary",
                                               NULL, NULL, MXML_DESCEND_ALL);
        if (rotNode) {
          const char *minStr = mxmlElementGetAttr(rotNode, "min");
          const char *maxStr = mxmlElementGetAttr(rotNode, "max");
          if (minStr)
            minRot = atof(minStr);
          if (maxStr)
            maxRot = atof(maxStr);
      ...
      mxml_node_t *homeNode = mxmlFindElement(
          assemblyNode, assemblyNode, "home", NULL, NULL, MXML_DESCEND_ALL);
      if (homeNode) {
        mxml_node_t *homePositionNode = mxmlFindElement(
            homeNode, homeNode, "position", NULL, NULL, MXML_DESCEND_ALL);
        if (homePositionNode) {
        ...
        mxml_node_t *homeRotationNode = mxmlFindElement(
            homeNode, homeNode, "rotation", NULL, NULL, MXML_DESCEND_ALL);
        if (homeRotationNode) {
      ...
      ucncAssembly *assembly = ucncAssemblyNew(
          name, parentName, originX, originY, originZ, positionX, positionY,
          positionZ, rotationX, rotationY, rotationZ, homePositionX,
          homePositionY, homePositionZ, homeRotationX, homeRotationY,
          homeRotationZ, colorR, colorG, colorB, motionType, motionAxis,
          invertMotion, minPos, maxPos, minRot, maxRot);

      if (assembly) {
        // Reallocation with error checking
        ucncAssembly **temp =
            realloc(assemblies, (assemblyCount + 1) * sizeof(ucncAssembly *));
        if (!temp) {
        ...
        if (limitsNode) {
          printf("  Limits: linear [%.2f, %.2f], rotary [%.2f, %.2f]\n", minPos,
                 maxPos, minRot, maxRot);
        }
      }
    }
  }
  ...
  // Process actors
  mxml_node_t *actorsNode =
      mxmlFindElement(tree, tree, "actors", NULL, NULL, MXML_DESCEND_ALL);
  if (actorsNode) {
    for (mxml_node_t *actorNode = mxmlFindElement(
             actorsNode, actorsNode, "actor", NULL, NULL, MXML_DESCEND_ALL);
         actorNode; actorNode = mxmlFindElement(actorNode, actorsNode, "actor",
                                                NULL, NULL, MXML_DESCEND_ALL)) {
      ...
      if (!name || !assemblyName || !stlFile) {
        fprintf(stderr,
                "Missing attribute (name, assembly, or stlFile) in actor.\n");
        continue;
      }

      // Process color
      float colorR = 1.0f, colorG = 1.0f, colorB = 1.0f;
      mxml_node_t *colorNode = mxmlFindElement(actorNode, actorNode, "color",
                                               NULL, NULL, MXML_DESCEND_ALL);
      if (colorNode) {
        colorR = atof(mxmlElementGetAttr(colorNode, "r"));
        colorG = atof(mxmlElementGetAttr(colorNode, "g"));
      ...
      ucncActor *actor =
          ucncActorNew(name, stlFile, colorR, colorG, colorB, configDir);
      if (!actor) {
  ...
  // Process lights
  // Process <lights> node
  mxml_node_t *lightsNode =
      mxmlFindElement(tree, tree, "lights", NULL, NULL, MXML_DESCEND_ALL);
  if (lightsNode) {
    // Iterate over each <light> node
    for (mxml_node_t *lightNode = mxmlFindElement(
             lightsNode, lightsNode, "light", NULL, NULL, MXML_DESCEND_ALL);
         lightNode; lightNode = mxmlFindElement(lightNode, lightsNode, "light",
                                                NULL, NULL, MXML_DESCEND_ALL)) {
      ...
      // Extract position
      mxml_node_t *positionNode = mxmlFindElement(
          lightNode, lightNode, "position", NULL, NULL, MXML_DESCEND_ALL);
      if (!positionNode) {
      ...
      if (!posX_str || !posY_str || !posZ_str) {
        fprintf(stderr,
                "Light '%s' has incomplete <position> attributes. Skipping.\n",
                lightID_str);
        continue;
      }
      ...
      mxml_node_t *ambientNode = mxmlFindElement(
          lightNode, lightNode, "ambient", NULL, NULL, MXML_DESCEND_ALL);
      if (!ambientNode) {
        fprintf(stderr,
                "Light '%s' missing <ambient> tag. Using default ambient "
                "(0,0,0).\n",
                lightID_str);
      }
      float ambientR =
          ambientNode ? atof(mxmlElementGetAttr(ambientNode, "r")) : 0.0f;
      float ambientG =
          ambientNode ? atof(mxmlElementGetAttr(ambientNode, "g")) : 0.0f;
      float ambientB =
          ambientNode ? atof(mxmlElementGetAttr(ambientNode, "b")) : 0.0f;

      // Extract diffuse color
      mxml_node_t *diffuseNode = mxmlFindElement(
          lightNode, lightNode, "diffuse", NULL, NULL, MXML_DESCEND_ALL);
      if (!diffuseNode) {
        fprintf(stderr,
                "Light '%s' missing <diffuse> tag. Using default diffuse "
                "(1,1,1).\n",
                lightID_str);
      }
      float diffuseR =
          diffuseNode ? atof(mxmlElementGetAttr(diffuseNode, "r")) : 1.0f;
      float diffuseG =
          diffuseNode ? atof(mxmlElementGetAttr(diffuseNode, "g")) : 1.0f;
      float diffuseB =
          diffuseNode ? atof(mxmlElementGetAttr(diffuseNode, "b")) : 1.0f;

      // Extract specular color
      mxml_node_t *specularNode = mxmlFindElement(
          lightNode, lightNode, "specular", NULL, NULL, MXML_DESCEND_ALL);
      if (!specularNode) {
        fprintf(stderr,
                "Light '%s' missing <specular> tag. Using default specular "
                "(1,1,1).\n",
                lightID_str);
      }
      float specularR =
          specularNode ? atof(mxmlElementGetAttr(specularNode, "r")) : 1.0f;
      float specularG =
          specularNode ? atof(mxmlElementGetAttr(specularNode, "g")) : 1.0f;
      float specularB =
          specularNode ? atof(mxmlElementGetAttr(specularNode, "b")) : 1.0f;

      // Create light object
      ucncLight *light = ucncLightNew(
          lightID, posX, posY, posZ, ambientR, ambientG, ambientB, diffuseR,
      ...
      mxml_node_t *spotNode = mxmlFindElement(lightNode, lightNode, "spot",
                                              NULL, NULL, MXML_DESCEND_ALL);
      if (spotNode) {
        mxml_node_t *directionNode = mxmlFindElement(
            spotNode, spotNode, "direction", NULL, NULL, MXML_DESCEND_ALL);
        if (!directionNode) {
          fprintf(stderr,
                  "Light '%s' has <spot> without <direction>. Ignoring "
          ...
          if (!dirX_str || !dirY_str || !dirZ_str) {
            fprintf(stderr,
                    "Light '%s' has incomplete <direction> attributes. "
                    "Ignoring spotlight properties.\n",
                    lightID_str);
          } else {
      ...
      mxml_node_t *attenuationNode = mxmlFindElement(
          lightNode, lightNode, "attenuation", NULL, NULL, MXML_DESCEND_ALL);
      if (attenuationNode) {
        mxml_node_t *constantNode =
            mxmlFindElement(attenuationNode, attenuationNode, "constant", NULL,
                            NULL, MXML_DESCEND_ALL);
        if (constantNode) {
        ...
        mxml_node_t *linearNode =
            mxmlFindElement(attenuationNode, attenuationNode, "linear", NULL,
                            NULL, MXML_DESCEND_ALL);
        if (linearNode) {
        ...
        mxml_node_t *quadraticNode =
            mxmlFindElement(attenuationNode, attenuationNode, "quadratic", NULL,
                            NULL, MXML_DESCEND_ALL);
        if (quadraticNode) {
          const char *quadratic_val_str =
              mxmlElementGetAttr(quadraticNode, "value");
      ...
      ucncLight **temp =
          realloc(loadedLights, (loadedLightCount + 1) * sizeof(ucncLight *));
      if (!temp) {
        fprintf(stderr, "Reallocation failed while loading lights. Freeing "
                        "allocated lights.\n");
        ucncLightFree(light);
  ...
  *lightCount = loadedLightCount;
  ...
  if (*rootAssembly == NULL) {
    ...
    *rootAssembly =
        ucncAssemblyNew("root", NULL, 0.0f, 0.0f, 0.0f, // Default origin
  ...
  free(assemblies); // Free the temporary assemblies array after setting the
                    // hierarchy
  return 1;         // Success
}
