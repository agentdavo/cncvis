...
/*
Strict out-of-memory checking. All OpenGL function calls are invalidated (ALL OF
THEM) if a GL_OUT_OF_MEMORY error occurs. This means that TinyGL has to
constantly check all gl_malloc() attempts for errors and the state of the error
state variable. The checks slow down the renderer so it is not recommended , but
...
/*
Use Fast Inverse Square Root. Toggleable because it's actually slower on some
platforms, And because some systems may have float types which are incompatible
with it.
*/
...
/*
!!!!!WARNING!!!!!
TGL_FEATURE_ALIGNAS assumes that the implementation's malloc (AND REALLOC) are
16-byte aligned.

Disabled by default for compatibility- you should only enable this if you KNOW
that the target ALWAYS returns 16-byte aligned pointers.
*/
...
2- Branching has extreme cost, allow a lot of extraneous code. Modern processors
work best on this setting.
*/
...
#include <stdalign.h>
#define TGL_ALIGN __attribute__((aligned(16)))
#else
