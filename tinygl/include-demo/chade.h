...
static inline GLuint loadRGBTexture(unsigned char *buf, unsigned int w,
                                    unsigned int h) {
  GLuint t = 0;
  glGenTextures(1, &t);
...
static inline GLuint createModelDisplayList(vec3 *points, GLuint npoints,
                                            vec3 *colors, vec3 *normals,
                                            vec3 *texcoords) {
  GLuint i, ret = 0;
  if (!points)
    return 0;
...
static inline void drawModel(vec3 *points, GLuint npoints, vec3 *colors,
                             vec3 *normals, vec3 *texcoords) {
  GLuint i;
  if (!points)
    return;
  glBegin(GL_TRIANGLES);
...
static inline void ChadWorld_RemoveEntityByPointer(ChadWorld *world,
                                                   ChadEntity *ent) {
  for (long i = 0; i < world->max_ents; i++)
    if (world->ents[i] == ent) {
      world->ents[i] = NULL;
...
static inline void renderChadWorld(ChadWorld *world) {
  long i;
  /*The user will already have set up the viewport, the camera, and the
   * lights.*/
