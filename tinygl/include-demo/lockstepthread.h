 ...
 *
 * A lightweight helper for running a worker thread in lock-step with
 * the main thread. Each call to `step()` releases the worker to execute
 * one iteration of `execute(argument)` while `lock()` waits for that
 * iteration to finish. Useful for deterministic multi-threading without
...
void step(lsthread *t) {
  if (t->state == -1)
    return; // if already stepping, nono
  if (!t->isThreadLive)
    return;
  // exit(1)
...
void start_lsthread(lsthread *t) {
  ...
  if (pthread_mutex_lock(&t->myMutex))
    exit(1);
  t->state = 1; // LOCKED
  pthread_create(&t->myThread, NULL, lsthread_func, (void *)t);
}
#endif
