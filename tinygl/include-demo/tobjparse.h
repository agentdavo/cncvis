...
static inline model tobj_tomodel(objraw *raw) {
  ...
  if (raw->normals)
    ret.n = malloc(sizeof(vec3) * raw->nfaces);
  if (raw->texcoords)
    ret.t = malloc(sizeof(vec3) * raw->nfaces);
  if (raw->colors)
    ret.c = malloc(sizeof(vec3) * raw->nfaces);
  long piter = 0;
...
static inline objraw tobj_load(const char *fn) {
  ...
  if (f) {
    char line[2048];
    line[2047] = 0;
#define TOBJ_PUSH(type, vec, n, val)                                           \
  {                                                                            \
    vec = realloc(vec, sizeof(type) * (n + 1));                                \
    vec[n++] = val;                                                            \
  }
