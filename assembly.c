...
// Implementation of ucncAssemblyNew, ucncAssemblyAddActor,
// ucncAssemblyAddAssembly, ucncAssemblyRender, ucncAssemblyFree
...
ucncAssemblyNew(const char *name, const char *parentName, float originX,
  ...
  if (parentName && strlen(parentName) > 0) {
    snprintf(assembly->parentName, sizeof(assembly->parentName), "%s",
             parentName);
  } else {
  ...
  if (motionType && strlen(motionType) > 0) {
    snprintf(assembly->motionType, sizeof(assembly->motionType), "%s",
             motionType);
  } else {
    snprintf(assembly->motionType, sizeof(assembly->motionType),
  ...
  assembly->minPos = minPos;
  assembly->maxPos = maxPos;
  assembly->minRot = minRot;
  assembly->maxRot = maxRot;
...
int ucncAssemblyAddActor(ucncAssembly *assembly, ucncActor *actor) {
  ...
  ucncActor **temp = realloc(assembly->actors,
                             (assembly->actorCount + 1) * sizeof(ucncActor *));
  if (!temp) {
    // If realloc fails, output error message and return failure
    fprintf(stderr,
            "Reallocation failed when adding actor '%s' to assembly '%s'.\n",
...
int ucncAssemblyAddAssembly(ucncAssembly *parent, ucncAssembly *child) {
  ...
  ucncAssembly **temp = realloc(
      parent->assemblies, (parent->assemblyCount + 1) * sizeof(ucncAssembly *));
  if (!temp) {
    // If realloc fails, output error message and return failure
    fprintf(stderr,
            "Reallocation failed when adding assembly '%s' to parent assembly "
            "'%s'.\n",
...
void ucncAssemblyRender(const ucncAssembly *assembly) {
  ...
  // printf("Rendering assembly: %s at position (%.2f, %.2f, %.2f) with rotation
  // (%.2f, %.2f, %.2f)\n",
  //       assembly->name, assembly->positionX, assembly->positionY,
  //       assembly->positionZ, assembly->rotationX, assembly->rotationY,
  //       assembly->rotationZ);
...
void ucncAssemblyFree(ucncAssembly *assembly) {
  ...
  for (int i = 0; i < assembly->assemblyCount; i++) {
    if (assembly->assemblies[i]) {
      ucncAssemblyFree(
          assembly->assemblies[i]); // Recursive call to free child assemblies
    }
  }
